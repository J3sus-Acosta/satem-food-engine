...
| **FASE 17**  | **Reportes Personalizados & Excel (.xlsx)** | Módulo completo `/dashboard/reports` con selección de columnas, filtros dinámicos, plantillas guardadas y exportación directa a Excel (`.xlsx`).   | ✅ Completada |
| **FASE 18**  | **Administración de Sucursales**            | Módulo completo `/dashboard/locations` para crear, editar, activar y desactivar sucursales (`Location`) reutilizando el modelo de datos existente. | ✅ Completada |
| **FASE 19**  | **Migración de Sucursales a SUPERADMIN**    | Introducción del rol `SUPERADMIN` con bypass/acceso global y restricción exclusiva de la administración de sucursales a `SUPERADMIN`.              | ✅ Completada |
