...
13. La anulación o devolución de ventas (total o parcial) requiere obligatoriamente autorización en tiempo real con credenciales de un usuario con rol `ADMIN` u `OWNER` (o permiso `sales.void`). Queda prohibido cerrar o cambiar la sesión activa del cajero en el navegador durante este proceso. Las anulaciones deben ser procesadas atómicamente en una transacción Prisma (`OrderVoidService.executeVoid`), recalculando montos netos proporcionales a descuentos previos, registrando egresos de caja (`CashMovement OUT`), devolviendo insumos de inventario (`StockMovement RETURN`) y guardando auditoría inmutable en `CashAudit` y `Order.metadata.voids`.
